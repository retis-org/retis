...
1. Release a new version on GitHub.
   ...
   1. Update the version name in `retis/src/main.rs`.
   1. Figure out if retis-events version needs to be bumped and if so, update
      `retis-events/Cargo.toml` and its reference in `retis/Cargo.toml`.
