...
If no collector is explicitly enabled (`--collectors` is unset) Retis will will
try to load all collectors if their individual requirements are met (e.g. the
`ovs` collector needs the OpenVSwitch kernel module to be loaded). If in
addition no probe is specified (`--probe` is unset) and no profile is used,
Retis will use a default set of probes (`net:netif_receive_skb` and
