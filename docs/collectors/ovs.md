...
The `--ovs-enrich-flows` option tells Retis is able to query the running OVS for this information
and add it to the event list as a new event section called `ovs-detrace`.
Then, `retis sort` will combine this event with the `flow_tbl_lookup` event to show extra information
of each flow hit.

Queries to OVS are throttled to 10 requests per second.
