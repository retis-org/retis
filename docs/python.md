...
Within the script execution context, a global variable called `event_file` is
available. It is of type `EventFile`.


``` python
$ cat myscript.py
for event in event_file.iter_events():
    if "skb" in event and "tcp" in event["skb"].raw():
        print("TCP event with dport: {}".format(
            event["skb"].tcp.dport)

$ retis events myscript.py
```
...
>>> event_file.sorted()
False
>>> reader = event_file.iter_events()
>>> reader
<builtins.EventReader object at 0x7f44e17eaf60>
>>> print("Got {} events".format(sum(1 for _ in reader))
Got 783 events
```
...
>>> event_file.sorted(
True
>>> reader = event_file.iter_series()
>>> reader
<builtins.SeriesReader object at 0x7f44e17e5b60>
>>> print("Got {} series".format(sum(1 for _ in reader))
Got 149 series
```
...
`EventReader` or `SeriesReader` can be created manually.
