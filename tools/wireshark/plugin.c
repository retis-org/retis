...
#define WIRESHARK_VERSION \
        ((WIRESHARK_VERSION_MAJOR * 100) + (WIRESHARK_VERSION_MINOR))

#if WIRESHARK_VERSION < 360
#error "Wireshark version 3.6 is required"
#endif
...
static const char *schema_file_path;
	...
	if (err) {
		if (errstr && tree) {
			expert_add_info_format(
				...
				proto_tree_add_item(tree, proto_retis, tvb, 0,
						    tvb_len, ENC_NA),
				&ei_retis_schema_parse_error, errstr);
		}
		g_free(errstr);
		return err;
	}
...
static void pref_retis_schema(void)
{
	...
	if (!f) {
		ws_debug("Failed to open configured schema file: %s", schema_file_path);
		return;
	}
