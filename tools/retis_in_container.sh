...
while [ $# -gt 0 ]; do
	...
	--allow-system-changes)
		...
		[ $runtime == "docker" ] && extra_args="--security-opt apparmor=unconfined"
		;;
	pcap)
		# We can't use a pseudo-tty (see `-t` option in `man podman
		# run`) when using a command outputting a specific format to
		# stdout (which could be piped into another utility parsing
		# it), like the pcap command. This is because an extra EOL char
		# is added (see commit 9f3361ac39c3).
		term_opts=""
		;;
	*)	;;
	esac
