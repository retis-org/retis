...
impl Collectors {
    ...
    fn init(&mut self, cli: &CliConfig) -> Result<()> {
        ...
        // If no profile is used and no probe is given, find the right set
        // automatically.
        if cli.main_config.profile.is_empty() && collect.args()?.probes.is_empty() {
            let mut probes = if collect.args()?.probe_stack {
