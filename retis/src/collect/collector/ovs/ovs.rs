...
impl OvsCollector {
    ...
    fn init_flow_enricher(
        ...
        let ovs_section_factory = section_factories
            .get_mut(&FactoryId::Ovs)
            .ok_or_else(|| anyhow!("ovs section factory not found"))?
            .as_any_mut()
            .downcast_mut::<OvsEventFactory>()
            .ok_or_else(|| anyhow!("Failed to downcast OvsEventFactory"))?;
