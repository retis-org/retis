            ...
            thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    while state.running() {
                        ...
                        if let Some(true) = ofpflows.first().map(|s| s.starts_with("Cache was not found")) {
                            // Openflow xlate cache is still not ready. This can happen if we are
                            // faster than the revalidator in populating it. Push it to the front
                            // of the task queue and retry later.
                            debug!(
