...
- A size: <LIMIT> is in bytes and should be suffixed with a size unit (MB, GB). The file names will be <OUT>.X, with X being a number starting at 0 and increasing over time. e.g. '--out-rotate 64MB'.

Events from the same series might end up on different files. If a previous collection with rotation enabled was not removed only the files required to store the new collection will be overridden."
    )]
