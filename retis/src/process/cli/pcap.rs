...
struct EventParser {
    /// Known network interfaces and their PCAP ids: key -> pcap id.
    /// "key" depends on whether probe_as_iface is set.
    ...
    wrote_header: bool,
    /// Whether to use probes as fake interfaces
    probe_as_iface: bool,
}
...
impl EventParser {
    ...
    fn new(probe_as_iface: bool) -> Self {
        Self {
            ifaces: HashMap::new(),
    ...
    /// Extract interface information from the event based on configuration,
    /// adding any necessary block and updating internal cache accordingly.
    fn process_interface(&mut self, event: &Event, blocks: &mut Vec<Block<'_>>) -> Result<u32> {
        let iface: String;
        let key: String;
        let desc: String;

        if self.probe_as_iface {
            iface = if let Some(kernel) = &event.kernel {
            ...
            key = iface.clone();
            desc = format!("Fake interface for probe {}", iface);
        } else {
            let (ifindex, ifname) = match &event.dev {
                Some(dev) => {
                    desc = format!("ifindex={}", dev.ifindex);
                    (dev.ifindex, dev.name.as_str())
                }
                None => {
                    desc = "Fake Interface".to_string();
            ...
            match &event.netns {
                Some(ns) => {
                    if let Some(cookie) = ns.cookie {
                        iface = format!("{ifname} ns {:#x}/{}", cookie, ns.inum);
                    } else {
                        iface = format!("{ifname} ns {}", ns.inum);
                    }
                    key = format!("{}/{ifindex}", ns.inum);
                }
                None => {
                    iface = ifname.to_string();
                    key = format!("0/{ifindex}");
    ...
    fn parse(&mut self, event: &mut Event) -> Result<Vec<Block<'_>>> {
        ...
        v.push(
            EnhancedPacketBlock {
                ...
                options: vec![
                    EnhancedPacketOption::Comment(Cow::Owned(comment)),
...
impl SubCommandParserRunner for Pcap {
    fn run(&mut self, _: &MainConfig) -> Result<()> {
        ...
        handle_events(
            self.input.as_path(),
            &filter,
            &mut EventParser::new(self.probe.is_none()),
            write_block,
        )?;
        Ok(())
    }
    ...
    fn test_handle_events() {
            ...
            (
                ...
                vec![
                    ...
                    Block::EnhancedPacket(EnhancedPacketBlock {
                        ...
                        original_len: 98,
                        options: vec![
                            EnhancedPacketOption::Comment(Cow::Owned("30419169082348 (6) [ping] 11330 [tp] net:netif_receive_skb #1baa83c42ba1ffff8e95c3b67c00 (skb ffff8e95d3009100)\n  192.168.125.10 > 192.168.125.11 tos 0x0 ttl 64 id 41977 off 0 [DF] len 84 proto ICMP (1) type 8 code 0\n  ns 0x1/4026531840 if 10 (veth-ns01-ovs)\n  skb [csum none len 84 priority 0 users 1 dataref 1]".to_string()
                            )),
                            EnhancedPacketOption::CustomUtf8(CustomUtf8Option {
                                code: 2988,
                                pen: RETIS_PEN,
            ...
            test_cases.into_iter()
        {
            ...
            match handle_events(
                Path::new(file_path),
                &filter,
                &mut EventParser::new(filter_symbol.is_empty()),
                write_blocks,
            ) {
