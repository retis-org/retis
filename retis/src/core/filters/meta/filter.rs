...
impl ParserMeta {
    ...
    fn parse_operator(pair: pest::iterators::Pair<Rule>) -> Result<RelOp> {
        match pair.as_str() {
            ...
            "<=" => Ok(RelOp::Le),
            "<" => Ok(RelOp::Lt),
            op => bail!("{op} is an invalid operator"),
        }
    }
    ...
    fn parse_infix(pair: pest::iterators::Pair<Rule>) -> Result<BooleanOp> {
        match pair.as_str() {
            ...
            "or" | "||" => Ok(BooleanOp::Or),
            _ => bail!("Unexpected boolean operator"),
        }
    }
    ...
    fn parse_term(pair: pest::iterators::Pair<Rule>) -> Result<AstNode> {
        ...
        for inner in inner_pairs {
            match inner.as_rule() {
                ...
                _ => bail!("unexpected terminal symbol"),
            }
        }
            ...
            Rule::expr => {
                ...
                for inner in inner_pair {
                    match inner.as_rule() {
                        ...
                        _ => bail!("while parsing the expression"),
                    }
                }
            }
            _ => bail!("unexpected rule while parsing expression"),
        }
...
impl<'a> MetaExpr<'a> {
    ...
    fn check_expr(&self, field: &LhsNode, rel_op: RelOp, rval: Rhs) -> Result<XlateCtx> {
        ...
        if ctx.load.is_ptr() || ctx.load.nmemb > 0 {
            ...
            if let Rhs::Str(val) = rval {
                ...
                ctx.target.sz = rval_len
                    .try_into()
                    .map_err(|_| anyhow!("invalid rval size (max {})", md.len() - 1))?;
        ...
        loop {
            self.filter.add(eBpfInsn::st(
                StInfo::Imm {
                    ...
                    imm: ctx.target.md[idx_sz as usize] as i32,
                },
                1.try_into()?,
            ));
    ...
    fn add_expr(&mut self, field: &LhsNode, relop: RelOp, rval: Rhs) -> Result<TFlist> {
        ...
        match sub_node {
            Some((offset, bfs, snode)) => {
                ...
                let mut ctx = self.check_expr(field, relop, rval)?;
                if let Some(bfs) = bfs {
