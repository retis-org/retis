...
fn retrieve_filter(code: u32) -> Vec<libbpf_sys::bpf_insn> {
    let f = if let Some(f) = get_filter(code) {
        match f {
            ...
            _ => {
                warn!("Found invalid type while while retrieving the filter, defaulting to match all ...");
                get_default_filter()
            }
        }
    } else {
...
// Rewrites the instructions, indexes all the pseduo calls, fixes
...
pub(crate) unsafe extern "C" fn fixup_filter_load_fn(
    ...
    let func_info_orig = copy_btf_info(
    ...
    let line_info_orig = copy_btf_info(
