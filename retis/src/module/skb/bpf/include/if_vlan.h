...
/**
 * vlan_tag_present tries to determine if a VLAN tag is present. There have been various changes over the years in
 * the kernel: The kernel uses skb_vlan_tag_present which in more recent versions either relies on vlan_present or on
 * vlan_all.
 * We use CO-RE functionality to probe if either field is present and return false otherwise.
 * See commit 354259fa73e2aac92ae5e19522adb69a92c15b49.
 *
 * @param skb
 *
...
/**
 * vlan_tag_present_old implements logic from before commit 0c4b2d370514cb4f3454dd3b18f031d2651fab73 to determine if a
 * VLAN tag is present. In old kernel versions, __vlan_hwaccel_put_tag set skb->vlan_tci = VLAN_TAG_PRESENT | vlan_tci.
 * Therefore, old versions of the kernel use the DEI / CFI bit indicate / detect presence of the VLAN tag.
 *
 * @param skb
 *
...
/**
 * __vlan_hwaccel_get_tag - get the VLAN TCI that is in @skb->cb[]. In newer versions of the kernel, report the vlan_tci
 * as is. Versions of the kernel before commit 0c4b2d370514cb4f3454dd3b18f031d2651fab73 always set the DEI / CFI bit to
 * on. As it is more common to have this bit off, and in order to not report a false positive, set the bit to 0 for 
 * these old kernels.
 *
 * @skb: skbuff to query
