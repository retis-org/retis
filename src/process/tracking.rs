...
impl AddTracking {
    ...
    // Add tracking information to an event based on skb tracking id.
    fn process_skb(&mut self, event: &mut Event) -> Result<Arc<Mutex<TrackingInfo>>> {
        let skb = event
            .get_section::<SkbTrackingEvent>(ModuleId::SkbTracking)
            .ok_or_else(|| anyhow!("No skb tracking id found"))?;
