...
/// is implemented for all event section types is `to_raw()`, which returns
...
impl PyEvent {
    ...
    /// event, eg. `e.to_raw()['skb']['dev']`.
    fn to_raw(&self, py: Python<'_>) -> PyResult<PyObject> {
        Ok(to_pyobject(
