...
impl<'a, 'inner> Formatter<'a, 'inner> {
    ...
    pub fn flush_buf(&mut self) -> result::Result<(), fmt::Error> {
        ...
        if first && self.level >= 2 {
            if let Some(item) = self.conf.item {
                prefix.replace_range(
                    ...
                    str::from_utf8(&[item as u8]).map_err(|_| fmt::Error)?,
                );
            }
        }
        ...
        if first && self.level >= 2 {
            prefix.replace_range(self.level - 2..self.level - 1, " ");
        }
...
/// This means a base configuration is can be shared for multiple
